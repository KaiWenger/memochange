...
pb_sim<-function(T,tau,trend=c("none","linear"),tp=0,d1,d2,mean,var){
  ...
  if((T1+T2)>T){T2<-T2-1}
  if((T1+T2)<T){T2<-T2+1}
  S1=LongMemoryTS::FI.sim(T1,q=1,rho=0,d=d1,var=var)+mean
  S2=LongMemoryTS::FI.sim(T2,q=1,rho=0,d=d2,var=var)+S1[length(S1)]
  series=c(S1,S2)
